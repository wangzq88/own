...
trait CTraitStyle
{
    ...
    public function one($top)
    {
        ...
        $n = imagettfbbox(28 / self::FONT_FORMAT, 0, $this->font_path, $goodsName);
        $n_width = $n[2] - $n[0];
        $n_height = abs($n[7] - $n[1]);

        $p = imagettfbbox(28 / self::FONT_FORMAT, 0, $this->font_path, $price);
        $p_width = $p[2] - $p[0];

        $o = imagettfbbox(26 / self::FONT_FORMAT, 0, $this->font_path, $original_price);
        $o_width = $o[2] - $o[0];
    ...
    public function two($top)
    {
        ...
        while ($goods = current($goodsList)) {
            ...
            $n = imagettfbbox(24 / self::FONT_FORMAT, 0, $this->font_path, $goodsName);
            $n_width = $n[2] - $n[0];
            $n_height = abs($n[7] - $n[1]);

            $p = imagettfbbox(24 / self::FONT_FORMAT, 0, $this->font_path, '￥' . $goods['price']);
            $p_width = $p[2] - $p[0];

            $o = imagettfbbox(22 / self::FONT_FORMAT, 0, $this->font_path, '￥' . $goods['original_price']);
            $o_width = $o[2] - $o[0];
    ...
    public function three($top)
    {
        ...
        while ($goods = current($goodsList)) {
            ...
            $price = '￥' . $goods['price'];
            $original_price = '￥' . $goods['original_price'];
//            $n = imagettfbbox(26 / self::FONT_FORMAT, 0, $this->font_path, $goodsName);
//            $n_height = abs($n[7] - $n[1]);
...
//            $n_height = abs($n[7] - $n[1]);
            ...
            $o = imagettfbbox(20 / self::FONT_FORMAT, 0, $this->font_path, $original_price);
            $o_width = $o[2] - $o[0];
