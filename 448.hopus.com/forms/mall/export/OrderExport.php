...
class OrderExport extends BaseExport
{
    ...
    protected function transform1($list)
    {
        $newList = [];
        ...
        foreach ($list as $item) {
            ...
            $arr = [];
            $arr['platform'] = $userPlatformDetail['platform'];
            ...
            $jiayouOrderInfo = JiayouOrder::find()->where(['order_id'=>$item->id])->asArray()->one();
            $jiayouInfo = Jiayou::find()->where(['id'=>$jiayouOrderInfo['jiayou_id']])->asArray()->one();
            $arr['qianghao'] = $jiayouInfo['qianghao'];
            $arr['youxing'] = $jiayouInfo['youxing'];
            $storeInfo = Store::find()->where(['id'=>$jiayouInfo['store_id']])->asArray()->one();
            $arr['jiayouName'] = $storeInfo['name'];
            $arr['integral_deduction_price'] = $item->integral_deduction_price;
            $arr['coupon_discount_price'] = $item->coupon_discount_price;
            $arr['js_discount_price'] = $jiayouOrderInfo['js_discount_price'];
