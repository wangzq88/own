...
class ShareOrderExport extends BaseExport
{
    ...
    {
        $query = $query->with(['user.userInfo', 'shareOrder', 'detail.share', 'clerkUser', 'store'])->orderBy(['created_at' => SORT_DESC]);

        $fileName = '分销订单列表' . \Yii::$app->mall->id;
        $filePath = \Yii::$app->basePath . '/web/csv/' . $fileName . '.csv';
        ...
        if ($this->page > $pagination->page_count) {
            $download_url = \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/csv/' . $fileName . '.csv?time=' . time();
            return [
