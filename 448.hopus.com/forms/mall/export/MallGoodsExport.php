...
class MallGoodsExport extends BaseExport
{
    ...
    public function export($query)
    {
        ...
        if (is_array($this->newDataList)) {
            ...
            $dirPath = \Yii::$app->basePath . '/web/csv/goods/' . \Yii::$app->mall->id . '/';
            if (!is_dir($dirPath)) {
            ...
            if (\Yii::$app->request->post('is_download')) {
                ...
                for ($i = 1; $i <= count($newDirs); $i++) {
                    $filePath = '/web/csv/goods/' . \Yii::$app->mall->id . '/' . $fileName . $i . '.csv';
...
//            $this->number = $number > 0 ? $number : 1;
            try {
                $this->transform($list);
                $dataList = $this->getDataList();
                (new \app\core\CsvExport())->ajaxExport($dataList, $this->fieldsNameList, $this->getFileName($fileName, $fileNum), '/web/csv/goods/' . \Yii::$app->mall->id . '/');
