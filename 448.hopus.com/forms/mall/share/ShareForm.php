...
class ShareForm extends Model
{
    ...
    public function getMa()
    {
        ...
        try {
            $autoInfo = CouponAutoSend::findOne(['id' => $this->id, 'is_delete' => 0]);

            if (!$autoInfo) {
                throw new \Exception('自动方案不存在');
            }
            $form = new CommonQrCode();
            $list = $form->getQrCodeMa(['auto_id' => $autoInfo->id]);

            if(!ApiCode::CODE_SUCCESS){
                $autoInfo->file_path = $list['file_path'];
                $autoInfo->save();
            }
