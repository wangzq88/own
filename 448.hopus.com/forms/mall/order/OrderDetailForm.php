...
class OrderDetailForm extends Model
{
    ...
    public function search()
    {
        ...
        if($order){
            $jiayouOrderInfo = JiayouOrder::find()->where(['order_id'=>$order['id']])->asArray()->one();
            if($jiayouOrderInfo){
                $order['js_discount_price'] = $jiayouOrderInfo['js_discount_price'];
            }else{
                $order['js_discount_price'] = 0;
            }
        }
