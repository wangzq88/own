...
abstract class BaseOrderForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['qianghao','yx','pay_type','brand_id'], 'safe'],
    ...
    public function search1()
    {
        ...
        $query = $this->getAllQuery1();

        if ($this->flag == "EXPORT") {
            $new_query = clone $query;
            $result = $this->export1($new_query);
            return $result;
        }
        ...
        if($list){
            foreach($list as $k=>$v){
                $jiayouOrderInfo = JiayouOrder::find()->where(['order_id'=>$v['id']])->asArray()->one();
                if($jiayouOrderInfo){
                    $list[$k]['js_discount_price'] = $jiayouOrderInfo['js_discount_price'];
                    $jiayouInfo = Jiayou::find()->where(['id'=>$jiayouOrderInfo['jiayou_id']])->asArray()->one();
                    if($jiayouInfo){
                        $storeInfo = Store::find()->where(['id'=>$jiayouInfo['store_id']])->asArray()->one();
                        $list[$k]['store_name'] = $storeInfo['name'];
                        $list[$k]['store_address'] = $storeInfo['address'];
                        $list[$k]['qianghao'] = $jiayouInfo['qianghao'];
                        $list[$k]['youxing'] = $jiayouInfo['youxing'];
                    }else{
                        $list[$k]['store_name'] = '';
        ...
        $add_export4 = [
            'key' => 'js_discount_price',
            'value' => '直降',
        ];
        $add_export5 =  [
                'key' => 'integral_deduction_price',
                'value' => '积分抵扣',
            ];
        $add_export6 =  [
                'key' => 'coupon_discount_price',
                'value' => '优惠券抵用',
            ];

        array_push($export_list,$add_export1);
        array_push($export_list,$add_export2);
        array_push($export_list,$add_export3);
        array_push($export_list,$add_export4);
        array_push($export_list,$add_export5);
        array_push($export_list,$add_export6);
    ...
    protected function export1($query)
    {
        ...
        $exp->send_type = $this->send_type;
        $exp->page = $this->page;
        return $exp->export1($query);
    }

    protected function exportOld($query)
    {
    ...
    protected function where1()
    {
        ...
        if ($this->pay_type){
            $query->andWhere(['o.pay_type'=>$this->pay_type]);
        }


        if($this->brand_id){
            $jiayouData = Jiayou::find()->where(['store_id'=>$this->brand_id])->asArray()->all();
            $jiayouIdArr = array_column($jiayouData,'id');
            $jiayouIdArr = array_unique($jiayouIdArr);
            $jiayouOrderData = JiayouOrder::find()->where(['in','jiayou_id',$jiayouIdArr])->asArray()->all();
            $orderIdArr =  array_column($jiayouOrderData,'order_id');
            $query->andWhere(['in','o.id',$orderIdArr]);
        }
        ...
        if($this->qianghao){
            $jiayouData = Jiayou::find()->where(['qianghao'=>$this->qianghao,'is_delete'=>0])->asArray()->all();
            $jiayouIdArr = array_column($jiayouData,'id');
            $jiayouIdArr = array_unique($jiayouIdArr);
            $jiayouOrderData = JiayouOrder::find()->where(['in','jiayou_id',$jiayouIdArr])->asArray()->all();
            $orderIdArr =  array_column($jiayouOrderData,'order_id');
        ...
        if($this->yx){
            $jiayouData = Jiayou::find()->where(['youxing'=>$this->yx,'is_delete'=>0])->asArray()->all();
            $jiayouIdArr = array_column($jiayouData,'id');
            $jiayouIdArr = array_unique($jiayouIdArr);
            $jiayouOrderData = JiayouOrder::find()->where(['in','jiayou_id',$jiayouIdArr])->asArray()->all();
            $orderIdArr =  array_column($jiayouOrderData,'order_id');
