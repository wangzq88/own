    ...
    public function balanceRecharge()
    {
        ...
        try {
            ...
            if ($this->id) {
                ...
                $order->jy_coupon_id = $recharge->jy_coupon_id;
                $order->pt_coupon_id = $recharge->pt_coupon_id;
                $order->jy_coupon_num = $recharge->jy_coupon_num;
                $order->pt_coupon_num = $recharge->pt_coupon_num;
            } else {
                $order->pay_price = $this->pay_price;
                $order->send_price = 0;
                $order->send_integral = 0;
                $order->send_member_id = 0;
                $order->jy_coupon_id = 0;
                $order->pt_coupon_id = 0;
                $order->jy_coupon_num = 0;
                $order->pt_coupon_num = 0;
            }
