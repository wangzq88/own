...
class CouponDetailForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['coupon_id', 'user_id','auto_id'], 'integer'],
        ];
    }
    ...
    public function receiveMa()
    {
        ...
        try {
            ...
            $autoInfo = CouponAutoSend::find()->where(['id'=>$this->auto_id])->one();
            if($autoInfo->is_delete == 1){
                return [
                    'code' => ApiCode::CODE_ERROR,
                    'msg' => '自动任务不存在'
                ];
            ...
            //判断是否指定了个别用户
            if($autoInfo->type==1) {
                if ($autoInfo->user_list) {
                    $user_arr = json_decode($autoInfo->user_list, true);
            ...
            $this->coupon_id = $autoInfo->coupon_id;
            ...
            if ($count >= $coupon->can_receive_count && $coupon->can_receive_count != -1) {
                ...
                $class = new UserCouponCenter($coupon, $common->user);
                if ($common->receive1($coupon, $class, '扫码领取')) {
                    if ($coupon->can_receive_count == -1) {
                        $rest = -1;
                    } elseif ($coupon->can_receive_count <= $count + 1) {
                        $rest = 0;
