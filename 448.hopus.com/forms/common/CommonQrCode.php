...
class CommonQrCode extends Model
{
    ...
    public function getQrCodeMa($scene = [], $width = 430, $page = 'pages/coupon/discount/discount')
    {
        try {
            $appPlatform = 'wxapp';
            ...
            if ($appPlatform == 'all') {
                ...
                return $this->wechat1($scene, $width, $page);
    ...
    private function wechat1($scene, $width = 430, $page = null)
    {
        ...
        if ($res->getStatusCode() == 200) {
            ...
            if (in_array('image/jpeg', $res->getHeader('Content-Type'))) {
                //返回图片
                $url = $this->saveTempImageByContent1($res->getBody()->getContents());
                return [
                    //TODO temp
                    'file_path' => $url,
                ];
    ...
    private function saveTempImageByContent1($content)
    {
        $imgName = md5(base64_encode($content)) . '.jpg';
        $save_path = \Yii::$app->basePath . '/web/couponma/' . $imgName;
        if (!is_dir(\Yii::$app->basePath . '/web/couponma')) {
            mkdir(\Yii::$app->basePath . '/web/couponma');
        }
        ...
        return \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/couponma/' . $imgName;
    }
