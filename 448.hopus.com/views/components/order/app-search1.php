...
<template id="app-search1">
    <div class="app-search">
        <div flex="wrap:wrap cross:center">
            ...
            <div class="label">加油站</div>
            <el-select size="small"  v-model="search.brand_id" class="select" @change="toSearch">
                <el-option key="0" label="全部加油站" value="0"></el-option>
                <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in brand"></el-option>
            </el-select>


            <div class="label">枪号</div>
            <el-select size="small"  v-model="search.qianghao" class="select" @change="toSearch">
                <el-option key="0" label="全部抢号" value="0"></el-option>
                <el-option :key="item.id" :label="item.qianghao" :value="item.qianghao" v-for="item in qianghaoArr"></el-option>
            </el-select>


            <div class="label">油型</div>
            <el-select size="small"  v-model="search.yx" class="select" @change="toSearch">
                <el-option key="0" label="全部" value="0"></el-option>
                <el-option :key="item.id" :label="item.youxing" :value="item.youxing" v-for="item in yxArr"></el-option>
            </el-select>
...
<script>
    Vue.component('app-search1', {
        template: '#app-search1',
        props: {
            selectList: {
                type: Array,
                default: function () {
        ...
        methods: {
            ...
            getBrand() {
                ...
                request({
                    params: {
                        r: 'plugin/scan_code_pay/mall/jiayou/store',
                    },
                    method: 'get',
                }).then(e => {
                    self.brand = e.data.data.list;
            ...
            getQh() {
                ...
                request({
                    params: {
                        r: 'plugin/scan_code_pay/mall/jiayou/qh',
                    },
                    method: 'get',
                }).then(e => {
                    self.qianghaoArr = e.data.data.list;
            ...
            getYx() {
                ...
                request({
                    params: {
                        r: 'plugin/scan_code_pay/mall/jiayou/yx',
                    },
                    method: 'get',
                }).then(e => {
                    self.yxArr = e.data.data.list;
