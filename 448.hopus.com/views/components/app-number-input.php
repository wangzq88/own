...
<template id="app-number-input">
    <div class="app-number-input-area" :class="pad ? 'pad': ''" flex="dir:top main:center">
        <div flex="dir:left cross:center" v-if="mode != 'password'">
            ...
            <el-input v-else @keyup.enter.native="confirmNumber" :ref="name" :type="mode == 'price' ? 'number': mode" class="app-member-input" size="small" :placeholder="placeholder" :autofocus="focus" :value="number" @input="getNumber" :style="{'width': inputWidth + 'px','margin' : margin +'px auto'}"></el-input>
        </div>
        <div v-if="mode == 'password'" style="text-align: center;color: #999999">请引导顾客输入6位数支付密码</div>
