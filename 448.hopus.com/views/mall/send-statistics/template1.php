...
Yii::$app->loadViewComponent('statistics/app-search1');
    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-area">
            <el-card v-loading="loading" shadow="never">
                ...
                <div class="num-info">
                    ...
                    <div class="num-info-item" v-for="(v,k) in all">
                        <div>{{v.youxing}}</div>
                        <div class="info-item-name">{{v.price}}元</div>
                    </div>
        ...
        <div class="table-body" style="padding: 20px">
            <el-table v-loading="loading" @sort-change="changeSort" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                ...
                <el-table-column prop="storeName" :label="search.type == 'coupon' ? '加油站':'加油站'">
                </el-table-column>
                <el-table-column prop="qianghao" label="枪号" sortable='custom' :label-class-name="chooseProp == 'all_num' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="youxing" :label="search.type == 'coupon' ? '油型':'油型'" sortable='custom' :label-class-name="chooseProp == 'use_num' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="coupon_discount_price" :label="search.type == 'coupon' ? '优惠券优惠':'优惠券优惠'" sortable='custom' :label-class-name="chooseProp == 'unuse_num' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="js_discount_price" :label="search.type == 'coupon' ? '直降':'直降'" sortable='custom' :label-class-name="chooseProp == 'unuse_num' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="total_pay_price" :label="search.type == 'coupon' ? '实付金额':'实付金额'" sortable='custom' :label-class-name="chooseProp == 'end_num' ? 'sort-active': ''">
                </el-table-column>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: '<?= $urlManager->createUrl('mall/send-statistics/index1')?>',
