...
import (
	"flag"
	"fmt"
	. "learn_go_kit/lesson14-midware-log/Service"
...
func main() {
	...
	endp := RateLimit(limit)(UserServiceLogMiddleware(logger)(GenUserEndPoint(user))) //调用限流代码生成的中间件

	options := []httptrans.ServerOption{ //生成ServerOtion切片，传入我们自定义的错误处理函数
		httptrans.ServerErrorEncoder(MyErrorEncoder),
	}

	serverHandler := httptrans.NewServer(endp, DecodeUserRequest, EncodeUserResponse, options...) //使用go kit创建server传入我们之前定义的两个解析函数
	//使用mux来使服务支持路由
